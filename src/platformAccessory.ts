...
export class ExamplePlatformAccessory {
  ...
  async getHeaterCoolerState(): Promise<CharacteristicValue> {
    ...
    if (deviceStatus.components) {
      const state = deviceStatus.components['main']['airConditionerMode']['airConditionerMode']['value'];
  ...
  async getCoolingTemperature(): Promise<CharacteristicValue> {
    const deviceStatus = await this.getStatus();
    if (deviceStatus.components) {
      const temperature = deviceStatus.components['main']['thermostatCoolingSetpoint']['coolingSetpoint']['value'];
      if (temperature) {
  ...
  async getCurrentTemperature(): Promise<CharacteristicValue> {
    const deviceStatus = await this.getStatus();
    if (deviceStatus.components) {
      const temperature = deviceStatus.components['main']['temperatureMeasurement']['temperature']['value'];
      if (temperature) {
        return temperature as number;
      }
    }
